...
class Graph(val nofVertices: Int, edges: Seq[(Int,Int)]) {
  ...
  def cutVertices: Set[Int] = {
    ...
    val depth = Array.ofDim[Int](nofVertices)
    val low = Array.ofDim[Int](nofVertices)
    val parent = Array.ofDim[Int](nofVertices)
    
    
    def inner(i: Int, d: Int): Unit = {
      visited(i) = true
      depth(i) = d
      low(i) = d
      var childcount = 0
      ...
      for(n <- neighbours(i)) {
        if(!visited(n)) {
          ...
          low(i) = scala.math.min(low(i), low(n))
        } else if (n != parent(i)) {
          low(i) = scala.math.min(low(i), low(n))
        }
      }
      if(i != 0 && isA || i == 0 && childcount > 1) {
        res = res + i
      }
   ...
   /* val visited = scala.collection.mutable.HashSet[Int]()
    val depth = Array.ofDim[Int](nofVertices)
    val lowpoint = Array.ofDim[Int](nofVertices)
    val parent = Array.ofDim[Int](nofVertices)
    val s = scala.collection.mutable.Stack[(Int, Iterator[Int])]()
    val isA = scala.collection.mutable.HashSet[Int]()
    var res = Set[Int]()
      
    
    var c = 0
    s.push((0, neighbours(0).iterator))
    visited(0) = true
    parent(0) = -1
    ...
    while (!s.isEmpty) {
      ...
      val n = s.top._1
      /*if(x != -1) {
        if(lowpoint(n) >= depth(x)) isA = true
      }*/
      if(s.top._2.hasNext) {
        val a = s.top._2.next
        ...
        if(!visited(a)) {
          parent(a) = n
          s.push((a, neighbours(a).iterator))
          visited(a) = true
          if (n == 0) c += 1
          depth(a) = s.size
          
          
          lowpoint(n) = scala.math.min(lowpoint(n), lowpoint(a))
        } else if (a != parent(n)) {
          lowpoint(n) = scala.math.min(lowpoint(n), depth(a))
        }
      } else { 
        
        
      /*  if(lowpoint(a) >= depth(n)) {
            isA(n) = true
          }*/
        if(n != 0 && isA(n) || n == 0 && c > 1) {
          res = res + n
        }
        s.pop
      }
