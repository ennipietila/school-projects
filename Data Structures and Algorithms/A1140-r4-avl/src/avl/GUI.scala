...
import collection.mutable.{ArrayStack, HashMap}
...
class GUI extends MainFrame {
  ...
  private val textAreaFont = new Font("monospaced", 0, 15)
  private val uiFont = new Font("monospaced", 0, 12)
  private val colorSucc = new Color(0,128,0)
  private val colorErr = new Color(178,34,34)
  ...
  private val leftOffsets, rightOffsets = Array.fill(renderMax)(0)
  ...
  private def renderTree(): Unit = {
    ...
    val stack = ArrayStack[Node[Int,Int]](node)
    ...
    setEdgeLengths(node)
    (0 until (nodeInfo(node.key).textHeight min renderMax)).foreach(leftOffsets(_) = infty)
    setOffsets(node, LeftDir)
    val c = -(0 until (nodeInfo(node.key).textHeight min renderMax)).foldLeft(0) { (res,i) =>
      res min leftOffsets(i)
    }
    ...
    def buildLevelWithEdges(node: Node[Int,Int], lvl: Int, col: Int): StringBuilder =  {
      ...
      val stack = ArrayStack[Frame](Frame(node, RootDir, lvl, col))
      while (stack.nonEmpty) {
        val f = stack.pop()
        if (f.n != null) {
        ...
        f match {
          ...
          case Frame(n, pd, _, _) => {
            if (!n.hasLeft && !n.hasRight)
              ...
              var limit = 0
              if (n.hasLeft) {
                (0 until (nodeInfo(n.left.key).textHeight min renderMax)).foreach(rightOffsets(_) = -infty)
                setOffsets(n.left, RightDir)
                limit = nodeInfo(n.left.key).textHeight
              }
              if (n.hasRight) {
                (0 until (nodeInfo(n.right.key).textHeight min renderMax)).foreach(leftOffsets(_) = infty)
                setOffsets(n.right, LeftDir)
                limit = limit min nodeInfo(n.right.key).textHeight
              } else
                limit = 0
              nodeInfo(n.key).edgeLength = (0 until limit).foldLeft(4) { (res,i) =>
                res max (rightOffsets(i) - leftOffsets(i) + 3)
              }/2
            }
            nodeInfo(n.key).textHeight = 1
            if (n.hasLeft)
              nodeInfo(n.key).textHeight =
                nodeInfo(n.key).textHeight max (nodeInfo(n.left.key).textHeight + nodeInfo(n.key).edgeLength + 1)
            if (n.hasRight)
              nodeInfo(n.key).textHeight =
                nodeInfo(n.key).textHeight max (nodeInfo(n.right.key).textHeight + nodeInfo(n.key).edgeLength + 1)
            pd match {
              case LeftDir => { stack.pop(); stack.top.rightDone = true }
              case RightDir => { stack.pop(); stack.top.leftDone = true }
              case _ => stack.pop()
  ...
  private def setOffsets(node: Node[Int,Int], dir: Direction): Unit = {
    require(dir == LeftDir || dir == RightDir)
    case class Frame(n: Node[Int,Int], l: Int, c: Int)
    val stack = ArrayStack[Frame](Frame(node, 0, 0))
    while (stack.nonEmpty) {
      val f = stack.pop()
      if (f.n != null) {
        dir match {
          case LeftDir => {
            ...
            leftOffsets(idx) = 
              leftOffsets(idx) min (f.c - (nodeInfo(f.n.key).asString.length() - adj)/2)
            if (f.n.hasLeft) {
              (1 to (nodeInfo(f.n.key).edgeLength min renderMax - f.l - 1)).foreach { i =>
                leftOffsets(f.l + i) = leftOffsets(f.l + i) min (f.c - i)
              }
            }
          }
          case RightDir => {
            ...
            val idx = f.l min renderMax - 1
            rightOffsets(idx) = 
              rightOffsets(idx) max (f.c + (nodeInfo(f.n.key).asString.length() - adj)/2)
            if (f.n.hasRight) {
              (1 to (nodeInfo(f.n.key).edgeLength min renderMax - f.l - 1)).foreach { i =>
                rightOffsets(f.l + i) = rightOffsets(f.l + i) max (f.c + i)
              }
            }
          }
        }
      ...
      try {
        tree.insert(k,v) match {
          case Some(_) => {
            ...
            Right("Successfully updated node " + k + "'s value with " + v)
          }
          case None => {
            nodeInfo += ((k, NodeAux(s"($k,$v)")))
            Right("Successfully added node " + k + " with value " + v)
          }
        }
      } catch {
    ...
    def attemptRemoval(k: Int): Either[Throwable, String] = {
      try {
        tree.remove(k) match {
          case Some(_) => {
            nodeInfo -= k
            Right("Successfully removed node with key " + k)
          }
          case None => throw SoftException("No node with key " + k + " exists")
        }
      } catch {
