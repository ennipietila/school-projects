...
class TreeMap[Key, Value](implicit ord: Ordering[Key]) {
  ...
  def leftRotation(x: Node[Key, Value]): Unit = {
    require(x.hasRight)
    var parent = x.parent
    var y = x.right
  ...
  def rightRotation(x: Node[Key, Value]): Unit = {
    require(x.hasLeft)
    var parent = x.parent
    var y = x.left
  ...
  def rebalance(node: Node[Key, Value]): Unit = {
    var x = node
  ...
  def insert(key: Key, value: Value): Option[Value] = {
    def inner(n: Node[Key, Value]): Option[Value] = {
      if(ord.compare(key, n.key) < 0) {
        if(n.hasLeft) {
          inner(n.left)
        } else {
          _nofKeys += 1
          n.left_=(new Node(key, value))
          rebalance(n)
          return None
        }
      } else if(ord.compare(key, n.key) > 0) {
        if(n.hasRight) {
          inner(n.right)
        } else {
          _nofKeys += 1
          n.right_=(new Node(key, value))
          rebalance(n)
          return None      
        }
      } else {
   ...
   */
  def min: Option[Key] = {
    def inner(n: Node[Key, Value]): Option[Key] = {
      if(n.hasLeft) {
        inner(n.left)
      } else {
        return Some(n.key)
