...
class Graph(val nofVertices: Int, edges: Seq[(Int,Int)]) {
  ...
  def eccentricity(v: Int): Int = {
    ...
    val seen = new Array[Boolean](nofVertices)
      val q = new scala.collection.mutable.Queue[Int]()
      val length = new Array[Int](nofVertices)
      q.enqueue(v)
      seen(v) = true
      var nofSeen = 1
      while(q.nonEmpty) {
        val x: Int = q.dequeue
        for(w <- neighbours(x) if !seen(w)) {
          q.enqueue(w)
          seen(w) = true
          nofSeen += 1
          length(w) = length(x) + 1
  ...
  def diameter: Int = {
    ...
    var max = eccentricity(0)
    for(v <- 1 until nofVertices - 1) {
      if(eccentricity(v) > max) max = eccentricity(v)
    }
    max
  }
