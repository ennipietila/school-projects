...
object solver {
  ...
  def solve(nofVertices: Int, edges: Seq[(Int, Int, Int)]): Option[Int] = {
    ...
    var ordered = edges.sortBy(-_._2)
    
    i = 0
    while(i < ordered.size){
      sets.union(ordered(i)._1, ordered(i)._3)
      if(sets.nofSets == 1) return Some(ordered(i)._2)
      i += 1
    }
    None
...
class UnionFind[E] {
  ...
  def union(element1: E, element2: E): Unit = {
    var x = findSet(element1)
    var y = findSet(element2)
    if(x != y) {
      _nofSets -= 1
      if(rank(x) < rank(y)) {
        parent(x) = y       
