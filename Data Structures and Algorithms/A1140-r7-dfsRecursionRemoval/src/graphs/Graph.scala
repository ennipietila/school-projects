...
final class Graph[V] {
  ...
  def pathFromDFSNonrec(source: V, targetPred: V => Boolean): Option[Seq[V]] = {
    ...
    path = source :: path
    visited(source) = true
    if(targetPred(source)) return Some(path)
    val h = scala.collection.mutable.HashMap[V, Iterator[V]]()
    getVertices.foreach(x => h(x) = neighbours(x).iterator)
    ...
    while(!path.isEmpty) {
      ...
      if (!n.hasNext) {
        ...
        if(!visited(a)) {
          ...
          path = a :: path
          if(targetPred(a)) return Some(path.reverse)
          visited(a) = true
        }
      }
...
object Graph {
   ...
   */
  def apply[V](edges: Traversable[(V, V)]): Graph[V] = {
    val g = new Graph[V]
  ...
  /**
   * Build an undirected graph from a map that associates vertices to sets of neighbours.
   */
  def apply[V](neighbourSets: Map[V, Traversable[V]]): Graph[V] = {
    val g = new Graph[V]
