...
object matcher {
  ...
  def fast[NutType <% Nut : ClassTag, BoltType <% Bolt : ClassTag](nuts: IndexedSeq[NutType], bolts: IndexedSeq[BoltType]): IndexedSeq[(NutType,BoltType)] = {
    var result: IndexedSeq[(NutType, BoltType)] = IndexedSeq()
    
    def inner(n: IndexedSeq[NutType], b: IndexedSeq[BoltType]): Unit = {
      val pivotnut = n(0)
      val groupedbolts = b.groupBy(_.compare(pivotnut))
      val pivotbolt = groupedbolts(0)(0)
      val groupednuts = n.groupBy(_.compare(pivotbolt))
      
      result = result ++ groupednuts(0).zip(groupedbolts(0))
      
      if (groupedbolts.isDefinedAt(-1)) {
        inner(groupednuts(-1), groupedbolts(-1))
      }
      if (groupedbolts.isDefinedAt(1)) {
        inner(groupednuts(1), groupedbolts(1))
      }      
