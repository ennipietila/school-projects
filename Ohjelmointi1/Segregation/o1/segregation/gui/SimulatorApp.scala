...
/** The singleton object `SimulatorApp` represents an application that runs  
  * Schelling segregation simulations in a GUI window and provides controls for 
  * adjusting the simulation settings.  
  *
  * '''NOTE TO STUDENTS: In this course, you don't need to understand how this object works 
  * or can be used, apart from the fact that you can use this file to start the program.''' */
object SimulatorApp extends App with O1SwingDefaults {
  ...
  val gui = new SimpleFrame("City Simulation") with Escapable with TerminatesOnClose {


  private val screen = new ViewComponent(simulator) with HasPauseToggle[Simulator] {
    ...
    val buttonRow = new FlowPanel { contents.append(stepButton, runButton, startButton) }
    ...
    val similaritySlider               = new Setting("Threshold:",              0,  70, 100, _ + "% similarity desired." ) with RestartsSim
    ...
    val vacancySlider        = new Setting("Vacancy:",        0,  10,  100, _ + "% vacant locations." ) with RestartsSim
    val sizeSlider           = new Setting("Grid size:",      1,  20,   70, side => side + " by " + side + " squares.") with RestartsSim 
