...
class Glass(val board: GameBoard, val location: GridPos) {
  ...
  /** Returns the number of neighboring booze glasses (diagonals included).
    ...
    * It does NOT change the level; that is the job of [[pourBooze]]. */
  def dangerLevel = this.danger
  
  
  /** Returns this glass's neighboring glasses (diagonals included) on the game board. */
  def neighbors: Vector[Glass] = this.board.neighbors(this.location, true) // TODO: implement this method
  ...
  def pourBooze() = {
   if (this.isWater) {
     this.isWater = false
     this.isFull = true
     this.neighbors.foreach( _.danger += 1 )
   }
  }
  ...
  /** Virtually drinks the contents of the glass and returns a value indicating
    ...
    *
    * In case of a glass of booze, "drinking" it not only sets the glass to be empty but 
    * also empties all the other glasses of booze on the board. (This ends the game
  ...
  def drink(): Boolean = {
    if (this.isWater) {
      ...
      if (this.dangerLevel == 0) {
        this.neighbors.filter(_.isFull).foreach(_.drink)
      }
      true
    } else {  
