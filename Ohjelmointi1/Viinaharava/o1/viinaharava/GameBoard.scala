...
class GameBoard(width: Int, height: Int, boozeCount: Int) extends Grid[Glass](width, height) {
  ...
  private def placeBoozeAtRandom(howMany: Int) = {
    Random.shuffle(allElements).take(howMany).foreach(_.pourBooze)
  }
  
  
  /** Determines whether all the water on the board has been drunk already. 
    * This is the case if all glasses on the board are either booze or empty. */ 
  def isOutOfWater: Boolean = this.allElements.forall(n => n.isEmpty || n.isBooze)// TODO: implement this method properly
