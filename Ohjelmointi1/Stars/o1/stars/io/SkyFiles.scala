...
import o1._
import o1.stars._
import o1.util.{localFile,readFileLines,readLinesFromSource,ifImplemented}
...
object SkyFiles {
  ...
  def parseStarInfo(starAsString: String): Star = { 
    val splitted = starAsString.split(";")
    val id = splitted(3).toInt
    val x = splitted(0).toDouble
    val y = splitted(1).toDouble
    val coords = new StarCoords(x, y)
    val magnitude = splitted(4).toDouble
    val name = {
      if (splitted.size == 7) {
        Some(splitted(6))
      } else None
      ...
      new Star(id, coords, magnitude, name)
  ...
  private def identifyConstellationFiles(folderPath: String, starListFileName: String): Vector[File] = { 
    val filesInFolder = for {
      actualFolder    <- localFile(folderPath)
      accessibleFiles <- Option(actualFolder.listFiles)
    } yield accessibleFiles.toVector
    filesInFolder.getOrElse(Vector.empty).filter( _.getName != starListFileName )
  }

  
  // Parses the given files, returning a constellation from each.
  private def readConstellations(files: Vector[File], allStars: Vector[Star]) = {   
    def entryFor(star: Star) = star.name.map( _ -> star ) 
    val starsByName = allStars.flatMap(entryFor).toMap
    for {
      file  <- files
      if allStars.nonEmpty
      lines <- Try(Source.fromFile(file)).toOption.map( readLinesFromSource(_) )
      starPairs = lines.map( parseConstellationLine(_, starsByName) )
      constellation <- ifImplemented(new Constellation(file.getName, starPairs))  
    } yield constellation
  }
  ...
  private def parseConstellationLine(line: String, starsByName: Map[String, Star]): (Star, Star) = { 
    ...
    val starPair = for {
      name1 <- tokens.lift(0)
      name2 <- tokens.lift(1)
      star1 <- starsByName.get(name1.trim)
      star2 <- starsByName.get(name2.trim)
    } yield (star1, star2)
    starPair.getOrElse(throw new RuntimeException(s"Invalid line in constellation definition: $line"))  
  }
}
