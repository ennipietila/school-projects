...
object SkyPic { 
  ...
  def placeStar(skyPic: Pic, star: Star): Pic = {
    val starimage = circle(12.0 / (star.magnitude + 2), White)     
    skyPic.place(starimage, star.posIn(skyPic))  
  ...
  def create(skyData: StarMap, bgSize: Int) = {
    ...
    skyData.constellations.foldLeft(darkSky)(placeConstellation)
  }
...
//The positions of the stars within the image are determined as in placeStar.
  ...
  def placeConstellation(skyPic: Pic, constellation: Constellation) = {
    val piste = circle(4, Red)
    var kuva = skyPic
    for (current <- constellation.lines) {
      kuva = line(current._1.posIn(skyPic), current._2.posIn(skyPic), Yellow).onto(kuva, current._1.posIn(skyPic))      
    }
    for (current <- constellation.stars) {
      kuva = piste.onto(kuva, current.posIn(skyPic))
    }
    kuva  
