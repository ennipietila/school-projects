...
class Constellation(val name: String, val lines: Vector[(Star,Star)]) {
  ...
  val stars: Set[Star] = (this.lines.unzip._1 ++ this.lines.unzip._2).toSet
  ...
  val minCoords = new StarCoords(this.stars.minBy(_.coords.x).coords.x , this.stars.minBy(_.coords.y).coords.y) // TODO: replace with a proper implementation (in Chapter 9.2)
  ...
  val maxCoords = new StarCoords(this.stars.maxBy(_.coords.x).coords.x, this.stars.maxBy(_.coords.y).coords.y) // TODO: replace with a proper implementation (in Chapter 9.2) 
