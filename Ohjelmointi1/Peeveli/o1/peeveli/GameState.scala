...
class GameState(val missesAllowed: Int, val previousGuesses: String, val visibleWord: String, val possibleSolutions: Vector[String])  {
  ...
  /** Creates a new `GameState` that represents the initial state of a new game of Peeveli.
    ...
    * in Chapter 3.6). You don’t need to use it. 
    *
  ...
  def isLost = missesAllowed < 0// TODO: replace with a proper implementation 
  ...
  def isWon = missesAllowed >= 0 && !this.visibleWord.contains('_')  // TODO: replace with a proper implementation 
  ...
  private def reveal(patternToReveal: String) = {
    var pattern = Buffer[Char]()
    for (index <- 0 until this.visibleWord.length) {
      if (this.visibleWord(index) == '_') {
        pattern += patternToReveal(index)
      } else {
        pattern += this.visibleWord(index)
      }
    }    
    pattern.mkString
  }
  ...
  def guessLetter(guess: Char) = {
    ...
    var largestgroup = this.possibleSolutions.groupBy( _.map(letter => if (letter == actualGuess) letter else '_') ).toVector.maxBy( _._2.size )
    var pattern = this.reveal(largestgroup._1)
    var possiblemiss = 0
    if (pattern == visibleWord) {
      possiblemiss = 1
    }
    new GameState(this.missesAllowed - possiblemiss, this.previousGuesses + actualGuess, pattern, largestgroup._2)      
  }
