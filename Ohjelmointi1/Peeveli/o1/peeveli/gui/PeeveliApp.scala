...
object PeeveliApp extends SimpleSwingApplication with O1SwingDefaults {
  ...
  def top = new MainFrame {
    ...
    def correctSolution() = {
      ...
      credibles(Random.nextInt(credibles.size))
    }
    ...
    private class PeeveliDisplay extends Label(" ") {
      ...
      def updateTo(state: GameState) = {
        val charsRevealed = revealOrder.size - state.missesAllowed - 1
