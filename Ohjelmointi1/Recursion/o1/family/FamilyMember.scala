...
class FamilyMember(val name: String, val children: Vector[FamilyMember]) {
  ...
  def numberOfDescendingGenerations: Int = {
    if (this.isChildless) {
      0
    } else if (this.children.forall(_.isChildless)) {
      1
    } else {  
      1 + this.children.map(_.numberOfDescendingGenerations).max
  ...
  def numberOfDescendants: Int = {
    if (this.isChildless) {
      ...
      this.numberOfChildren + this.children.map(_.numberOfDescendants).sum
    }    
  }
  ...
  /** Returns the number of descendants the `FamilyMember` has in a particular generation  
   ...
   def numberOfDescendantsAt(generation: Int): Int = {
     if (generation == 1) {
       ...
       this.children.map(_.numberOfChildren).sum       
     } else {
       this.children.map(_.numberOfDescendantsAt(generation - 1)).sum
  ...
  def mostChildren: FamilyMember = {
    if (this.isChildless) {
      ...
      this.children.map(_.mostChildren).maxBy(_.numberOfChildren)
    }    
  }
