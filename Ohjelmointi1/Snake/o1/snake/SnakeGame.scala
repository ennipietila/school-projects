...
class SnakeGame(initialPos: GridPos, initialHeading: CompassDir) {
  ...
  def isOver = {  
    ...
    val collidedWithWall = head.x <= 0 || head.x >= SizeInSquares || head.y <= 0 || head.y >= SizeInSquares
    val collidesWithItself = this.segments.tail.contains(this.segments.head)
    collidedWithWall || collidesWithItself
  ...
  def advance() = {
    var pos = this.segments.head.neighbor(snakeHeading)
    if (pos == nextFood) {      
      this.nextFood = randomLocationOnGrid()   
      segments = pos +: this.segments
    } else {
      segments = pos +: segments.init
