...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir) { 
  ...
  var brain: Option[RobotBrain] = None              // most-recent holder (it is possible for robot to change brains, but this we only do in Chapter 12.1)
  ...
  def neighboringSquare(direction: CompassDir): Square = {
    world.elementAt(this.location.neighbor(direction)) 
  }
  ...
  def spinClockwise() = {
    this.facesTowards = this.facesTowards.clockwise// TODO: implementation missing
  } 
  ...
  def canMoveTowards(direction: CompassDir) = {
    this.neighboringSquare(direction) != Wall
  }
  ...
  def isStuck = {
    ...
    directions.forall( !this.canMoveTowards(_) )
  }
