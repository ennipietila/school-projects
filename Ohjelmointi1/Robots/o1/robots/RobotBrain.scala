...
abstract class RobotBrain(initialName: String, val body: RobotBody) {
  ...
  /** Returns the coordinates that point at the square that is immediately in front 
    * of this robot. */
  def locationInFront = this.location.neighbor(this.body.facing)
  
  
  /** Returns the square that is immediately in front of this robot. */
  def squareInFront = this.body.neighboringSquare(this.body.facing)
  ...
  def robotInFront = {
    this.world.elementAt(this.locationInFront).robot.map( _.brain ).getOrElse(None)
