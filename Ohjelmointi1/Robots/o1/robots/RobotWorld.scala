...
/** An instance of the class `RobotWorld` represents a mutable, two-dimensional world  
  ...
  * @see [[Square]] */
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2) { // TODO: add "extends ..."   
  ...
  private def initialSquare(x: Int, y: Int): Square = {
    if (x == floorWidth + 1 || x == 0 || y == 0 || y == floorHeight + 1) {      
      Wall 
    } else {
      new Floor 
    }
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) = {
    val robot = new RobotBody(this, initialLocation, initialFacing)
    this.robots += robot
    this.robots.head
    robot.locationSquare.addRobot(robot)
    robot
  }
  ...
  def advanceTurn() = {
    this.nextRobot.foreach( _.takeTurn )
    if (robots(turn) == robots.last) {
      turn = 0
    } else {
      turn += 1
    }
  ...
  /** Causes all the robots in the world to take a turn, starting with the one whose turn it is next.
    ...
    * @see [[RobotBody.takeTurn]] */
  def advanceFullTurn() = {
    val order = robots.dropWhile( _ != nextRobot.get) ++ robots.takeWhile( _ != nextRobot.get )
    order.foreach( _.takeTurn )
  }
