...
object olioita {
  ...
  object tili {
    ...
    def talleta(maara: Int) = {
      this.saldo = this.saldo + max(maara, 0)
    }
    
    def nosta(nosto: Int) = {
      val saldoaluksi = saldo
      this.saldo = this.saldo - nosto - min(this.saldo - nosto, 0)
      nosto + min(saldoaluksi - nosto, 0)
    }
