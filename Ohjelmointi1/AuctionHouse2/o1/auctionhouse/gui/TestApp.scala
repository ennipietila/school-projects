...
object TestApp extends SimpleSwingApplication with O1SwingDefaults {
  ...
  def top = new MainFrame {
    ...
    def intParam(prompt: String) = {
      requestInt(prompt + ": ", _ >= 0, "Please enter a reasonable integer value.", RelativeTo(buttonRow)).map( new Integer(_) )
    }
    ...
    private class EAPanel(item: EnglishAuction) extends ItemPanel(item) {
      ...
      val bidButton = new Button("Place bid") {
        ...
        reactions += {
          case ButtonClicked(_) =>
            val result = EAPanel.this.item.bid(customerSelector.selection.item, Try(amountField.text.toInt).getOrElse(0))
            if (!result) {
