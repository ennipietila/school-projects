...
/** The class `TribalBot` represents the "brains" (or AI) of robots that belong to 
  ...
  *    a [[Frame]] object; the frames are stored as the elements of a [[Stack]]. This enables 
  *    the tribal bot to execute subprograms and return to the correct line in the tribe's 
  *    RoboSpeak program whenever the end of a subprogram is reached.
  *
...
class TribalBot(body: RobotBody, val tribe: Tribe) extends RobotBrain(tribe.newName(), body) {
  ...
  /** A tribal robot prioritizes hacking: if there is an enemy present directly in front,
    ...
    * @see [[Instruction]] */
  def moveBody() = {
    if (enemyInFront.isDefined && !this.tribe.isPacifist) {
      this.hack
    } else {      
      do {
        nextInstruction = nextInstruction.execute(this)
  ...
  def determineTribe(another: RobotBrain): Option[Tribe] = {
    another match {
      case joku: TribalBot => Some(joku.tribe)
      case _ => None
  ...
  def isFriend(robot: RobotBrain): Boolean = {
    Some(this.tribe) == determineTribe(robot) 
  }
  ...
  def talk(lineNumber: Int) = {
    if (this.seesFriend) {
      this.friendInFront.get.boot(lineNumber)
    }
  }
  ...
  def hack() = {
    if (this.seesEnemy) {
      this.enemyInFront.get.body.brain = Option(new TribalBot(enemyInFront.get.body, this.tribe))      
    }
  }
  ...
  def returnFromSubprogram() = {
    ...
    0 // TODO: provide a working implementation; call failUnderlowError in case the stack is empty
  }


  /** Returns the brains of all the robots in the same robot world with this one. */
  def longRadar: Vector[RobotBrain] = {
    this.world.robotList.filter(_.brain.isDefined).map(_.brain.get)
  }
  ...
  def directedRadar(dirNumber: Int) = {
    val targetDir = Stream.iterate(this.facing)( _.clockwise )(dirNumber)
