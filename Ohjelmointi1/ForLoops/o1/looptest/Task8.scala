...
// This file is associated with Chapter 5.4.
...
object Task8 extends App {
  ...
  // the right is the length of a hypotenuse and the two integers on the right are 
  ...
  //  println(first + " and " + second + " -----> " + hypot(first, second))
  
  for (first <- 1 to upperLimit) {
    for (second <- 1 to upperLimit) {
      if (second >= first) {
        println(first + " and " + second + " -----> " + hypot(first, second))
      }
    }
