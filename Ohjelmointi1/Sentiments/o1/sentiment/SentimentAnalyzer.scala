...
class SentimentAnalyzer private(private val sentimentsOfWord: Map[String, WordSentiment]) { 
  ...
  def rate(wordOrPhrase: String) = {
    val averagesOfFamiliarWords = wordsOf(wordOrPhrase).flatMap( this.sentimentsOfWord(_).average ) 
...
object SentimentAnalyzer {
  ...
  def fromSource(trainingData: Source) = {
    ...
    def parse(line: String): Try[(Int, Seq[String])] = { 
      val (sentimentString, review) = line.span( char => !char.isWhitespace )
      Try(sentimentString.toInt).map( initialNumber => (initialNumber, wordsOf(cleanText(review))) )  
    }
    ...
    def addLine(line: String) = parse(line) match {
      case Success((humanAssignedRating, words)) => 
