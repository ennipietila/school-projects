...
object FootballApp extends SimpleSwingApplication with O1SwingDefaults {
    ...
    class SeasonPanel(val owner: Window) extends EasyPanel {
      ...
      def addMatch() = {
        for {
          ...
          for (newMatch <- dialog.finishedMatch; season <- this.season) {
            withStudentSolution(SeasonPanel.this) {
              season.addResult[Unit](classOf[Match] -> arg(newMatch))
              this.updateStatus()
      ...
      private def updateStatus() = {
        for (season <- this.season) {
          val matchText =  if (season.numberOfMatches[Int] == 1) "1 match" else (season.numberOfMatches[Int] + " matches") 
      ...
      class EnhancedSeason(wrapped: AnyRef) extends DynamicObject(wrapped) { 
        def apply(n: Int) = this.matchNumber[Option[Match]](classOf[Int] -> arg(n))
