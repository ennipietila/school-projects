...
object factory {
  ...
  def build5BitLFSR(state: Bus): Bus = {
    ...
    state(4).buildFeedback((state(0) && !state(2)) || (!state(0) && state(2)))
    state(0).set(true)
    state
  }
  ...
  def buildLFSR(taps: List[Int], state: Bus): Bus = {
    ...
    for (i <- 0 until taps.length) {
      current = current && !state(taps(i)) || !current && state(taps(i))
    }
    if (taps.isEmpty) {
      state(l - 1).buildFeedback(state(0))
    } else {
      state(l - 1).buildFeedback(current)
    }
    state
  }
}
