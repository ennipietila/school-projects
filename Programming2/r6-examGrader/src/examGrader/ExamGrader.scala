...
class ExamGrader(val studentDB: StudentDB,
  ...
  lazy val graded: Set[StudentId] = {
    homeworkDone.filter(n => examPoints.contains(n)).toSet
  }
  ...
  def pointsToGrade(n: Int): Int = {
    if (n < gradeLimits(0)) {
      0
    } else if (n >= gradeLimits(0) && n < gradeLimits(1)) {
      1
    } else if (n >= gradeLimits(1) && n < gradeLimits(2)) {
      2
    } else if (n >= gradeLimits(2) && n < gradeLimits(3)) {
      3
    } else if (n >= gradeLimits(3) && n < gradeLimits(4)) {
      4
    } else {
      5
    }
  ...
  def points(id: StudentId): Int = {
    require(examPoints.contains(id))
      examPoints.apply(id).sum + bonusPoints.getOrElse(id, 0)
  ...
  lazy val grades: Map[StudentId, Int] = {
    graded.map(n => (n, pointsToGrade(points(n)))).toMap             
  }
  ...
  def bestStudents(n: Int): Seq[StudentId] = {  
    graded.to[collection.immutable.Seq].sortWith(points(_) > points(_)).take(n)
  }
  ...
  def studentsByGrade: Map[Int, Set[StudentId]] = {
    val grades = List(0, 1, 2, 3, 4, 5)   
    val students = grades.map(x => graded.filter(n => pointsToGrade(points(n)) == x))
    grades.zip(students).toMap
  }
  ...
  def gradedProgrammes: Set[StudyProgramme] = {
    graded.map(x => studentDB.programmeOf(x))
  }
  ...
  def gradesByProgramme: Map[StudyProgramme, Map[StudentId, Int]] = {
    gradedProgrammes.map(x => (x, grades.filter(n => studentDB.programmeOf(n._1) == x))).toMap
