...
object run {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val path = "/Users/ennipietila/Documents/"
     ...
     * Once you have the VALUE of the identifier, save it in "solutions.scala".
     *
     */

    val task1 : String = balances.map(x => (x._2, x._1)).sortByKey(false).first._2 // your CODE here
     ...
     * Once you have the VALUE of the volume, save it in "solutions.scala".
     *
     */

    val task2 : Long = transactions.map(x => x._4).reduce(_ + _) // your CODE here
    ...
    val task3 : String = {
      val out = transactions.map(x => (x._2, - x._4)) // your CODE here
      val in = transactions.map(x => (x._3, x._4))
      balances.union(out).union(in).reduceByKey(_ + _).map(x => (x._2, x._1)).sortByKey(false).first._2
    }
     ...
     * Once you have the VALUE, save it in "solutions.scala".
     *
     */

    val task4 : Long = {
      val joku = transactions.map(x => (x._2, 1)).union(transactions.map(x => (x._3, 1)))
      balances.map(x => (x._1, 0)).union(joku).reduceByKey(_ + _).filter(_._2 == 0).count// your CODE here
    }
    ...
    val task5 : Long = {
      transactions.map(x => ((x._1 - start_inclusive) / 3600000, x._4)).reduceByKey(_ + _).filter(_._2 >= 1000000000L).count
    }
    ...
    val task6 : Long = 234 // your CODE here
    ...
    val task7 : Long = 3543 // your CODE here
