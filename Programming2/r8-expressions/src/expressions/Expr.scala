...
abstract class Expr {
  ...
  final def partialDerivative(v: String): Expr = this match {
    case Var(n) => if (n == v) Num(1.0) else Num(0.0)
    case Num(_) => Num(0.0)
    case Multiply(a, b) => a.partialDerivative(v) * b + b.partialDerivative(v) * a
    case Add(a, b) => a.partialDerivative(v) + b.partialDerivative(v)
    case Subtract(a, b) => a.partialDerivative(v) - b.partialDerivative(v)
    case Negate(a) => -a.partialDerivative(v)
  }
  ...
  final def expand: Expr = {
    def negate(joku: Expr): Expr = joku match {
      case Var(n) => Negate(Var(n))
      case Num(n) => Num(-n)
      case Negate(n) => n
      case Multiply(a, b) => Multiply(negate(a), b)      
    }
    def list(jee: Expr): List[Expr] = jee match {
      case Var(n) => List(Var(n))
      case Num(n) => List(Num(n))
      case Negate(n) => list(n).map(negate(_))
      case Add(a, b) => list(a) ++ list(b)
      case Subtract(a, b) => list(a) ++ list(b).map(negate(_))
      case Multiply(a, b) => for(p1 <- list(a); p2 <- list(b)) 
        yield Multiply(p1, p2)    
    }
    list(this).reduce(_ + _)
  }
}
