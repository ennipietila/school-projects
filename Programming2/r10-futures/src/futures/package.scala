...
package object futures {
   ...
   *
   */
  def sumF(x: Future[Int], y: Future[Int]): Future[Int] = for (r1 <- x; r2 <- y) yield r1 + r2 
  ...
  object ServicesF {
    ...
    def B(x: Future[Int]): Future[Int] = x.map(2 * _)
    def C(x: Future[Int]): Future[Int] = x.map(_ + 1)
    def D(x: Future[Int]): Future[Int] = x.map(-_)
    def E(x: Future[Int]): Future[Int] = x.map(3 * _)
    def P(x: Future[Int], y: Future[Int]): Future[Int] = (x zip y).map(n => n._1 + n._2)
    def Q(x: Future[Int], y: Future[Int]): Future[Int] = (x zip y).map(n => n._1 * n._2)
    }
  ...
  def eitherF(x: Future[Int], y: Future[Int]): Future[Int] = x.fallbackTo(y).recover{case _ => y.value.get.get}
