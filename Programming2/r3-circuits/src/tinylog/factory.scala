...
object factory {
  ...
  def buildEqual(bus1: Bus, bus2: Bus): Gate = {
    ...
    bus1.zip(bus2).map(n => n._1 && n._2 || !n._1 && !n._2).reduce( _ && _ )
  }
  ...
  def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    var current = Gate.False
     for (i <- 0 until bus1.length) {
       val equal = bus1(i) && bus2(i) || !bus1(i) && !bus2(i)
       val less = !bus1(i) && bus2(i)
       val both = current && equal
       current = both || less
     }
     current   
  }
  ...
  def buildIncrementer(bus: Bus): Bus = {
    ...
    var carry_in: Gate = Gate.True 
    var uusi = new Array[Gate](bus.length)
  ...
  def buildCountTrueBits(bus: Bus): Bus = {
    ...
    if (bus.length == 1) {
      ...
      val joku = buildHalfAdder(bus(0), bus(1))
      Bus(joku._2, joku._1)
    } else if (bus.length == 3) {
      val joku = buildFullAdder(bus(0), bus(1), bus(2))
      Bus(joku._2, joku._1)      
    } else {
      var overflow = bus.splitAt(bus.length / 2)
      buildAdderNoOverflow(buildCountTrueBits(overflow._1), buildCountTrueBits(overflow._2)).take(log2ceil(bus.length))
    }
  }
  ...
  def buildMultiplier(bus1: Bus, bus2: Bus): Bus = {
    ...
    val newlength = bus1.length + bus2.length
    var joku = Bus.falses(newlength)
    for (i <- 0 until bus1.length) {
       joku = buildAdderNoOverflow(Bus.falses(i) ++ (bus2 && bus1(i)) , joku)
    }
    joku.take(newlength)
  }
}
