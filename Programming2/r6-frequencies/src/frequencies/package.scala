...
package object frequencies {
  ...
  def frequencies[T](seq: Seq[T]): Map[T, Int] = {
    seq.map(x => (x, seq.filter(_ == x))).toMap.mapValues(n => n.size)
  }
  ...
  def freqToItems[T](seq: Seq[T]): Map[Int, Set[T]] = {
    val joku = frequencies(seq).groupBy(x => x._2)
    joku.mapValues(x => x.keys.toSet)
  }
  ...
  def mostFrequent[T](seq: Seq[T]): Set[T] = {
    if (seq.isEmpty) {
      ...
      val joku = freqToItems(seq).keys.max 
      freqToItems(seq).apply(joku)
    }
  }
}
