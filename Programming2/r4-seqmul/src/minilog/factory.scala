...
object factory {
  ...
  def buildSequentialMultiplier(aa: Bus, bb: Bus, loadEnable: Gate): (Gate, Bus) = {
    val c = loadEnable.host
    val result = c.inputs(aa.length + bb.length)
    val aaa = c.inputs(result.size)
    val bbb = c.inputs(result.size)
     
    result.buildFeedback(buildAdder(result, aaa && bbb(0)) && !loadEnable)
    aaa.buildFeedback((aa ++ c.falses(bb.size) && loadEnable) | ((aaa.host.falses(1) ++ aaa.dropRight(1)) && !loadEnable))
    bbb.buildFeedback((bb ++ c.falses(aa.size) && loadEnable) | ((bbb.drop(1) ++ bbb.host.falses(1)) && !loadEnable))
    val ready = (((~aaa).reduceLeft(_&&_)) || ((~bbb).reduceLeft(_&&_))) && !loadEnable
