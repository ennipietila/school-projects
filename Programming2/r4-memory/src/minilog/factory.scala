...
object factory {
  ...
  def buildOneWordMemory(readEnable: Gate, writeEnable: Gate, data: Bus): Bus = {
    ...
    var state = readEnable.host.inputs(data.length)
    state.buildFeedback(buildTwoBusSelector(state, data, writeEnable))
    state && readEnable
  }
  ...
  def buildMemory(readEnable: Gate, writeEnable: Gate, address: Bus, data: Bus): Bus = {
    ...
    require(data.length > 0, "Data bus must have positive width.")
    buildDecoder(address).map(n => buildOneWordMemory(readEnable && n, writeEnable && n, data)).reduce(_ | _)
