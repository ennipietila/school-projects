...
abstract class LinkedList[A]() {
   ...
   */
   final def count(p: A => Boolean): Int = {
     @tailrec def inner(l: LinkedList[A], result: Int ): Int = {
      l match {
        case Nil() => result
        case Cons(h, t) => if (p(h)) inner(t, result + 1) else inner(t, result)
      }
    }
    inner(this, 0)
   }
   ...
   */
   final def filterNot(p: A => Boolean): LinkedList[A] = {
    @tailrec def inner(l: LinkedList[A], result: LinkedList[A]): LinkedList[A] = {
      l match {
        case Nil() => result
        case Cons(h, t) => if (p(h)) inner(t, result) else inner(t, Cons(h, result))
      }
    }
   ...
   */
  final def last: A = {
    @tailrec def inner(l: LinkedList[A]): A = {
      if (l.tail.isEmpty) l.head else {
        inner(l.tail)
      }
    } 
    inner(this)
  ...
  final def drop(n: Int): LinkedList[A] = {
    @tailrec def inner(l: LinkedList[A], count: Int): LinkedList[A] = {
      if (count <= n) {
        l match {
          case Nil() => Nil()
          case Cons(h, t) => inner(t, count + 1)  
        }
      } else {
        l
      }      
  ...
  final def map[B](f: A => B): LinkedList[B] = {
    @tailrec def inner(l: LinkedList[A], result: LinkedList[B]): LinkedList[B] = {
      l match {
        case Nil() => result
  ...
  final def foldLeft[B](start: B)(f: (B, A) => B): B = {
    @tailrec def inner(l: LinkedList[A], result: B): B = {
      l match {
        case Nil() => result
        case Cons(h, t) => inner(t, f(result, h))
      }
    }
    inner(this, start)
  }
  ...
  final def splitAt(n: Int): (LinkedList[A], LinkedList[A]) = {
    @tailrec def inner(alku: LinkedList[A], loppu: LinkedList[A], luku: Int): (LinkedList[A], LinkedList[A]) = {
      loppu match {
        case Nil() => (alku.reverse, Nil())
        case Cons(h, t) => if (luku > 0) inner(Cons(h, alku), t, luku - 1) else (alku.reverse, loppu)
      }
    }
    inner(Nil(), this, n)
