...
object run {
  ...
  def main(args: Array[String]): Unit = {
   ...
   //val filename = path ++ "pg2600.txt"
     ...
     * http://www.scala-lang.org/api/2.12.2/scala/collection/immutable/StringOps.html
     *
     */

    val depunctuated_lines: RDD[String] = {
      lines.map(line => line.filter(_ != ',').filter(_ != '.').filter( _ !=':').filter( _ !=  ';').filter( _ != '\"').filter( _ != '(').filter( _ !=  ')').filter( _ !=  '{').filter( _ !=  '}'))
    }
    ...
    val tokens: RDD[String] = depunctuated_lines.flatMap(line => line.split(" ")).filter(line => line.length > 0)                          // transform 'depunctuated_lines' to tokens
    ...
    val tokens_lc: RDD[String] =  tokens.map(token => token.toLowerCase)                  // map each token in 'tokens' to lower case
    ...
    val words: RDD[String] = tokens_lc.filter(token => token.forall(alphabet.contains(_)))                            // filter out all but words from 'tokens_lc'
    ...
    val word_counts: RDD[(Long,String)] = words.map(v => (v.toString, 1.toLong)).foldByKey(0)(_ + _).map(x => (x._2, x._1)).sortByKey(false)
