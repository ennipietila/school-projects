...
package object sequences {
  ...
  def mySum(in: Seq[Int]): Int = in.reduce((a, b) => a + b)
  /*
   * Task 4:
  ...
  def sumOfSquaresOfOdd(in: Seq[Int]): Int =
    in.filter(_ % 2 == 1)         // (1)  [when making a cascade it is convenient ..]
      .map(n => n * n)            // (2)  [.. to break the cascade into multiple]
      .reduce((a, b) => a + b)         // (3)  [.. lines to ease readability]
  ...
  def innerProductOfTwoDDimVectors(a: Seq[Int], b: Seq[Int]): Int =
    (a zip b).map(n => n._1 * n._2).reduce((a, b) => a + b)
  ...
  def squareOfDDimEuclideanDistance(a: Seq[Int], b: Seq[Int]): Int =
    (a zip b).map(n => n._2 - n._1).map(n => n * n).reduce((a, b) => a + b)
  ...
  def movingSum(in: Seq[Int], w: Int): Seq[Int] = {
    ...
    var joku = in.scanRight(0)((a, b) => a + b)
    joku.zip(joku.drop(w)).map(n => n._1 - n._2)
    /*joku.take(in.length - w + 1).map(n => n - in.drop(joku.indexOf(n) + w).sum)*/
  }
}
