...
package object promises {
  ...
  def buildPlusF(): (Promise[Int], Promise[Int], Future[Int]) = {
    val p = Promise[Int]
    val q = Promise[Int]
    val f = (p.future zip q.future).map(n => n._1 + n._2)
    (p, q, f)
  }
  ...
  def buildPlusZeroFailF(): (Promise[Int], Promise[Int], Future[Int]) = {
    val p = Promise[Int]
    val q = Promise[Int]
    val f = (p.future zip q.future).map(n => n._1 + n._2).recover { case _ => 0 }
    (p, q, f)
  }
   ...
   * http://www.scala-lang.org/api/2.12.2/scala/concurrent/Promise.html
   *
   */
  def fastestOfTwo(a: Future[Int], b: Future[Int]): Future[Int] = {
    val p = Promise[Int]
    p.tryCompleteWith(b).tryCompleteWith(a).future
   ...
   * http://www.scala-lang.org/api/2.12.2/scala/concurrent/Future.html
   *
   */
  def fastestSuccessfulOfTwo(a: Future[Int], b: Future[Int]): Future[Int] = {
    val p = Promise[Int]
    a.foreach(x => p.trySuccess(x))
    b.foreach(x => p.trySuccess(x))
    a.failed.foreach(x => b.failed.foreach(y => p.tryFailure(x)))
    p.future
  }
  ...
  def buildTimesFast0F(): (Promise[Int], Promise[Int], Future[Int]) = {
    ...
    val q = Promise[Int]
    val f = Promise[Int]
    p.future.foreach(x => if (x == 0) f.trySuccess(0) else f.tryCompleteWith((p.future zip q.future).map(n => n._1 * n._2)))
    q.future.foreach(x => if (x == 0) f.trySuccess(0) else f.tryCompleteWith((p.future zip q.future).map(n => n._1 * n._2)))
    p.future.failed.foreach(x => q.future.failed.foreach(y => f.tryFailure(x)))
    (p, q, f.future)
  }
}
